...
export async function createUser(
	...
	if (res.status() != 200)
		console.error(
			`User ${email} creation failed with status ${res.status()}`,
...
export async function authenticateUser(context: BrowserContext, email: string, password: string) {
	const fbUser = (await signInWithEmailAndPassword(auth, email, password)).user;
